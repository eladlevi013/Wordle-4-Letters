...
const random = Math.floor(Math.random() * common_words.length);
let target_word = common_words[random];
// let target_word = 'ABBA';
...
async function closePopup()
{
    ...
    await sleep(500);
    document.getElementById('popup1').style.position = 'static';
}

function openPopup()
{
    document.getElementById('popup1').style.position = 'fixed';
...
let tryNumber=0; // this is the start of the line
var focused_button_id = 0; // the current focused button
...
async function onEnterButtonClick(){
    ...
    if(checkInputInWordlist(inputWord) == true){
        ...
        if(focused_button_id < 19){
            let next_inputField = document.getElementById(focused_button_id);
            next_inputField.focus();
        }
        
        let letters_in_the_word = [];
        let bull_letters=0;
        let colored_cells = [0,0,0,0]; // 0-grey, 1-yellow, 2-green 
        ...
        for(let i=0; i<inputWord.length; i++)
        {
            ...
            {
                if(inputWord[i] == target_word[j])
                {
                    if(i == j)
                    {
                        //document.getElementById((tryNumber-4)+i).style.backgroundColor = "#6AAA64";
                        colored_cells[i] = 2;

                        letters_in_the_word.push(inputWord[i]);
                        bull_letters += 1;
                        //alert(bull_letters);
            ...
            if(letters_in_the_word.includes(inputWord[i]) == false && letter_found == true)
            {
                //document.getElementById((tryNumber-4)+i).style.backgroundColor = "#C9B458";
                colored_cells[i] = 1;
            }
        }
        // after the guess, we have the color the cells
        for(let i=0; i<4; i++){
            let color;
            if(colored_cells[i] == 0) color = '#3A3A3C';
            else if(colored_cells[i] == 1) color = '#C9B458';
            else if(colored_cells[i] == 2) color = '#6AAA64';

            document.getElementById(tryNumber-4+i).animate([
                // keyframes
...
function KeyCheck(event)
{
   ...
   else if((event.keyCode >= 65 && event.keyCode <= 90) || (event.keyCode > 96 && event.keyCode < 123))
   {
        if(focused_button_id < tryNumber+3) { // if the cell if right
            if(document.getElementById(focused_button_id).value.length > 0){ // if the wrote letter, continue to next one
                // focuse of the next button id
                if(focused_button_id < 19) {
                    // console.log("!!!");
                    let next_inputField = document.getElementById(focused_button_id+1);
                    next_inputField.focus();
                    focused_button_id += 1;
                }
            }
        }
        // console.log(focused_button_id);
   }
}
*/
    ...
    else if((event.keyCode >= 65 && event.keyCode <= 90) || (event.keyCode > 96 && event.keyCode < 123))
   {
        if(focused_button_id < tryNumber+3) { // if the cell if right
            if(document.getElementById(focused_button_id).value.length > 0){ // if the wrote letter, continue to next one
                // focuse of the next button id
                if(focused_button_id < 19) {
                    document.getElementById(focused_button_id+1).focus();
                    focused_button_id += 1;
                }
            }
        }
        console.log("good: " + focused_button_id);
   }
});
...
function checkInputInWordlist(inputWord){
    for(let i=0; i<all_words.length; i++)
    {
        if(inputWord == all_words[i]) return true;
    }
    return false;
}

function backslashPress()
{
    if(focused_button_id >= tryNumber) {
        if(focused_button_id == tryNumber){
            document.getElementById(focused_button_id).value = '';
        }
        else {
            document.getElementById(focused_button_id).value = '';
            focused_button_id = focused_button_id - 1;
            document.getElementById(focused_button_id).focus();
        }
    }
}
...
{
    if(focused_button_id < tryNumber+3) { // if the cell if right 
        if(document.getElementById(focused_button_id).value.length > 0){
            if(focused_button_id < 19) {
                document.getElementById(focused_button_id+1).focus();
                focused_button_id += 1;
                document.getElementById(focused_button_id).value = letter;
            }
        }
        else if(document.getElementById(focused_button_id).value.length == 0) {
            document.getElementById(focused_button_id).value = letter;
        }
    }
